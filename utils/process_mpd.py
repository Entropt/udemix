...
import shutil
import subprocess
from constants import logger, remove_emojis_and_binary, timestamp_to_seconds
...
def download_and_merge_mpd(
    ...
    # Pass the MPD URL directly to n_m3u8dl-re so it can resolve
    # relative segment URLs. Downloading to a local file loses the
    # base URL context, causing "Download init file failed!" errors.
    process_mpd(
        mpd_file_url,
        download_folder_path,
        title_of_output_mp4,
...
def process_mpd(
    ...
    nm3u8dl_command = (
        f'n_m3u8dl-re "{mpd_url}" --save-dir "{download_folder_path}" '
    ...
    if process_nm3u8dl.returncode != 0:
        if stderr_nm3u8dl:
            logger.error(
                f"n_m3u8dl-re error for {remove_emojis_and_binary(output_file_name)}: {stderr_nm3u8dl.strip()}"
            )
    ...
    if process_ffmpeg.returncode != 0:
        if stderr_ffmpeg:
            logger.error(
                f"ffmpeg error for {remove_emojis_and_binary(output_file_name)}: {stderr_ffmpeg.strip()[:500]}"
            )
        progress.console.log(

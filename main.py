...
class Udemy:
    def __init__(self):
        ...
        try:
            ...
            logger.critical(
                f"The provided cookie file could not be read or is incorrectly formatted. Please ensure the file is in the correct format and contains valid authentication cookies."
            )
            sys.exit(1)
    ...
    def request(self, url):
        try:
            ...
            logger.critical(
                f"There was a problem reaching the Udemy server. This could be due to network issues, an invalid URL, or Udemy being temporarily unavailable."
            )
    ...
    def extract_course_id(self, course_url):
        ...
        if meta_match:
            ...
            if number_match:
                ...
                logger.critical(
                    "Unable to retrieve a valid course ID from the provided course URL. Please check the course URL or try with --id."
                )
                sys.exit(1)
        else:
            logger.critical(
    ...
    def fetch_lecture_info(self, course_id, lecture_id):
        try:
            return self.request(
                LECTURE_URL.format(course_id=course_id, lecture_id=lecture_id)
            ).json()
        except Exception as e:
    ...
    def download_lecture(
        ...
        lecture_title = f"{lindex}. {sanitize_filename(lecture['title'])}"
        ...
        if not skip_lectures and lect_info["asset"]["asset_type"] == "Video":
            video_path = os.path.join(folder_path, lecture_title + ".mp4")
            if os.path.isfile(video_path) and os.path.getsize(video_path) > 0:
                progress.console.log(
                    f"[yellow]Skipped (already exists) {remove_emojis_and_binary(lecture_title)}[/yellow]"
                )
                if os.path.isdir(temp_folder_path):
                    shutil.rmtree(temp_folder_path)
            else:
                # Remove partial/incomplete file from a previous interrupted download
                ...
                if mpd_url is None:
                    if m3u8_url is None:
                        if mp4_url is None:
                            logger.error(
                                f"This lecture appears to be served in different format. We currently do not support downloading this format. Please create an issue on GitHub if you need this feature."
                            )
                        else:
                            download_mp4(
                                mp4_url,
                    ...
                    if key is None:
                        logger.warning(
                            "The video appears to be DRM-protected, and it may not play without a valid Widevine decryption key."
                        )
            ...
            article_path = os.path.join(folder_path, lecture_title + ".html")
            if os.path.isfile(article_path) and os.path.getsize(article_path) > 0:
                progress.console.log(
                    f"[yellow]Skipped (already exists) {remove_emojis_and_binary(lecture_title)}[/yellow]"
                )
                if os.path.isdir(temp_folder_path):
    ...
    def download_course(self, course_id, curriculum):
        ...
        with (
            ThreadPoolExecutor(max_workers=max_concurrent_lectures) as executor,
            Live(progress, refresh_per_second=10),
        ):
            task_generator = (
                (
            ...
            for _ in range(max_concurrent_lectures):
                try:
                    ...
                    # Clean up stale temp folder from a previous interrupted run
                    if os.path.isdir(temp_folder_path):
                        shutil.rmtree(temp_folder_path)
...
def check_prerequisites():
    if not cookie_path:
        if not os.path.isfile(os.path.join(HOME_DIR, "cookies.txt")):
            logger.error(
                f"Please provide a valid cookie file using the '--cookie' option."
            )
            return False
    else:
...
def main():
    try:
        ...
        if args.concurrent > 25:
            logger.warning(
                "The maximum number of concurrent downloads is 25. The provided number of concurrent downloads will be capped to 25."
            )
            max_concurrent_lectures = 25
        elif args.concurrent < 1:
            logger.warning(
                "The minimum number of concurrent downloads is 1. The provided number of concurrent downloads will be capped to 1."
            )
        ...
        if not course_url and not args.id:
            logger.error(
                "You must provide either the course ID with '--id' or the course URL with '--url' to proceed."
            )
            return
        ...
        if key is not None and not ":" in key:
            logger.error(
                "The provided Widevine key is either malformed or incorrect. Please check the key and try again."
            )
            return
        ...
        if args.captions:
            try:
                ...
                logger.error(
                    "Invalid captions provided. Captions should be separated by commas."
                )
        else:
            captions = ["en_US"]
        ...
        if args.load:
            if args.load is True and os.path.isfile(
                os.path.join(HOME_DIR, "course.json")
            ):
                try:
                    course_curriculum = json.load(
                        open(os.path.join(HOME_DIR, "course.json"), "r")
                    )
                    logger.info(
                        f"The course curriculum is successfully loaded from course.json"
                    )
                except json.JSONDecodeError:
                    logger.error(
                        "The course curriculum file provided is either malformed or corrupted."
                    )
                    sys.exit(1)
            elif args.load:
                if os.path.isfile(args.load):
                    try:
                        ...
                        logger.info(
                            f"The course curriculum is successfully loaded from {args.load}"
                        )
                    except json.JSONDecodeError:
                        logger.error(
                            "The course curriculum file provided is either malformed or corrupted."
                    ...
                    logger.error(
                        "The course curriculum file could not be located. Please verify the file path and ensure that the file exists."
                    )
                    sys.exit(1)
            else:
        ...
        if args.save:
            if args.save is True:
                if os.path.isfile(os.path.join(HOME_DIR, "course.json")):
                    logger.warning(
                        "Course curriculum file already exists. Overwriting the existing file."
                    )
                with open(os.path.join(HOME_DIR, "course.json"), "w") as f:
                    json.dump(course_curriculum, f, indent=4)
                    logger.info(
                        f"The course curriculum has been successfully saved to course.json"
                    )
            elif args.save:
                ...
                with open(args.save, "w") as f:
                    json.dump(course_curriculum, f, indent=4)
                    logger.info(
                        f"The course curriculum has been successfully saved to {args.save}"
                    )
        ...
        if args.tree:
            ...
            if args.tree is True:
                ...
                with open(args.tree, "w") as f:
                    ...
                    logger.info(
                        f"The course curriculum tree has been successfully saved to {args.tree}"
                    )
